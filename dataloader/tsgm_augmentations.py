...
"""
    Codes from: https://github.com/AlexanderVNikitin/tsgm/blob/main/tsgm/models/augmentations.py
    docs: https://tsgm.readthedocs.io/en/latest/
    tutorials: https://github.com/AlexanderVNikitin/tsgm/blob/main/tutorials/augmentations.ipynb
"""

AugmentationOutput = Union[np.ndarray, Tuple[np.ndarray, np.ndarray]]
...
class BaseAugmenter:
    def __init__(self, per_feature: bool) -> None:
        self.per_channel = per_feature

    def _get_seeds(self, total_num: int, n_seeds: int) -> np.ndarray:
        if n_seeds > total_num:
            seeds_idx = np.random.choice(range(total_num), size=n_seeds, replace=True)
        else:
            seeds_idx = np.random.choice(range(total_num), size=n_seeds, replace=False)
        return seeds_idx

    def generate(self, X: np.ndarray, y: Optional[np.ndarray] = None, n_samples: int = 1) -> AugmentationOutput:
...
class GaussianNoise(BaseAugmenter):
    ...
    def __init__(self, per_feature: bool = True) -> None:
        super(GaussianNoise, self).__init__(per_feature=per_feature)

    def generate(self, X: np.ndarray, y: Optional[np.ndarray] = None, n_samples: int = 1, 
                 mean: float = 0.0, variance: float = 1.0) -> AugmentationOutput:
        """
        Generate synthetic data with Gaussian noise.

        :param X: Input data array of shape (n_data, n_timesteps, n_features).

        :param y: Optional labels array. If provided, labels will also be returned
        ...
        :return: Augmented data array of shape (n_samples, n_timesteps, n_features) and 
                optionally augmented labels if 'y' is provided.
        """
...
class SliceAndShuffle(BaseAugmenter):
    ...
    """
    def __init__(self, per_feature: bool = False) -> None:
        super(SliceAndShuffle, self).__init__(per_feature=per_feature)

    def generate(self, X: np.ndarray, y: Optional[np.ndarray] = None,
                  n_samples: int = 1, n_segments: int = 2) -> AugmentationOutput:
        """
...
class MagnitudeWarping(BaseAugmenter):
    ...
    def generate(self, X: np.ndarray, y: Optional[np.ndarray] = None, 
            n_samples: int = 1, sigma: float = 0.2, n_knots: int = 4) -> AugmentationOutput:
        """
        ...
        if has_labels:
            return result, result_y.astype(int)
        else:
            return result
...
class WindowWarping(BaseAugmenter):
    ...
    def generate(self, X: np.ndarray, y: Optional[np.ndarray] = None, window_ratio: float = 0.2, 
                 scales: Union[List[float], Tuple[float, float]] = (0.25, 1.0), n_samples: int = 1) -> AugmentationOutput:
        """
        Generates augmented samples via MagnitudeWarping for (X, y)
        ...
        :param scales: A list specifying the scale range for warping.
            Default is [0.25, 1.0].
        ...
        # 根据scales的值, 如果是list就转成tuple
        assert len(scales) > 0 and len(scales) <= 2
...
class DTWBarycentricAveraging(BaseAugmenter):
    ...
    def generate(
        ...
        initial_timeseries: Optional[List[np.ndarray]] = None,
        ...
        **kwargs,
        ...
        Parameters
        ----------
        X : np.ndarray, the timeseries dataset
        y : np.ndarray or None, the classes
